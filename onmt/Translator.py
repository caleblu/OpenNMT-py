...
class Translator(object):
    def __init__(self, opt, dummy_opt={}):
        ...
        # alpha = Variable(torch.FloatTensor(2,1))
        # gender_direction = Variable(torch.FloatTensor(1,500))
    ...
    def _runTarget(self, batch, data,alpha=None,gender_direction=None):
        ...
        encStates, context = self.model.encoder(src, alpha,gender_direction,src_lengths)
    ...
    def translateBatch(self, batch, dataset,alpha=None,gender_direction=None):
        ...
        src = onmt.IO.make_features(batch, 'src')
        encStates, context = self.model.encoder(src,alpha,gender_direction,src_lengths)
        #encStates (2*1*500,2*1*500), #context(5*5*500)
        decStates = self.model.decoder.init_decoder_state(
                                        src, context, encStates)
        ...
        for b in beam:
            ...
            for i, (times, k) in enumerate(ks[:n_best]):
                hyp, att , out_list = b.getHyp(times, k)
                out_list_b.append(out_list)
                hyps.append(hyp)
                attn.append(att)
            allHyps.append(hyps)
            allScores.append(scores)
            allallScores.append(b.allScores)
            allAttn.append(attn)
            all_out_list.append(out_list_b)

        return allHyps, allScores, allAttn, allGold, allallScores,all_out_list

    def translate(self, batch, data,alpha,gender_direction):
        #  (1) convert words to indexes
        batch_size = batch.batch_size

        #  (2) translate
        pred, predScore, attn, goldScore,_,_ = self.translateBatch(batch, data,alpha,gender_direction)
